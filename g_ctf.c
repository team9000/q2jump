...
"if 16 "
  "xv 72 "
  // draxi - commented out the Antiglue indicator, since it's not used anymore! Also lowered the other 3 indicators..
  //"yb -8 " // 
  //"stat_string 16 "
  "yb -8 "
  "stat_string 25 "
...
void SetCTFStats(edict_t *ent)
{
	...
	if (ent->client->resp.rep_racing)
		...
		else if (ent->client->resp.rep_race_number == MAX_HIGHSCORES) {
			ent->client->ps.stats[STAT_JUMP_RACE] = CONFIG_JUMP_RACE_NOW;
		}
		else {
	...
	if (ent->client->resp.cleanhud)
	{
		...
		if (!ent->client->resp.replaying)
		{
			...
			ent->client->ps.stats[STAT_JUMP_KEY_JUMP] = CONFIG_JUMP_EMPTY;
			ent->client->ps.stats[STAT_JUMP_KEY_CROUCH] = CONFIG_JUMP_EMPTY;
			ent->client->ps.stats[STAT_JUMP_KEY_ATTACK] = CONFIG_JUMP_EMPTY; //draxi
		}
		return;
	}
...
//		sort_users();
	...
	case ELECT_ADDTIME :
		...
		// draxi - fixed the votetime bug!
		timeleft = ((mset_vars->timelimit + map_added_time) - ((int)(level.time / 60)));
		if ((map_added_time<0) && (map_added_time<(-timeleft))){
			End_Jumping();
			return;
		}

		// draxi - timeleft was "((int)(level.time / 60)) + (mset_vars->timelimit + map_added_time))"
		// wich made it go upwards, so changed it to "((mset_vars->timelimit + map_added_time) - ((int)(level.time / 60)))"
		Update_Added_Time();
		timeleft = ((mset_vars->timelimit + map_added_time) - ((int)(level.time / 60)));
		ctfgame.election = ELECT_NONE;
