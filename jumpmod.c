...
{ 
   ...
   gi.cprintf (ent, PRINT_HIGH, "--------------------------------------------------\n"); 
	gi.cprintf (ent, PRINT_HIGH, "ŒÔÆ Œ·ÌÂ             ±ÛÙ ≤Ó‰ ≥Ú‰ ¥ÙË µÙË ∂ÙË ∑ÙË ∏ÙË πÙË ±∞ÙË ”„ÔÚÂ\n"); 
   ...
   { 
	  ...
	  if (temp>=0)
	  {
		  ...
		  Com_sprintf(txt,sizeof(txt),"%-3d %-16s %3d %3d %3d %3d %3d %3d %3d %3d %3d  %3d %5d", i+1, name,
			  maplist.users[temp].points[0],
			  maplist.users[temp].points[1],
			  maplist.users[temp].points[2],
			  maplist.users[temp].points[3],
...
void Cmd_Replay(edict_t *ent)
{
	...
	if (strcmp(temp,"now")==0)
	{
		if (level_items.recorded_time_frames[MAX_HIGHSCORES])
		{
			...
			gi.cprintf(ent,PRINT_HIGH,"No time set on this map.\n");
		}
	}
	else
...
//draxi: Delete times without deleting top 10 each map..
void delete_nottop10(edict_t *ent)
{
	int remnum,i,nottop;
	FILE	*f;
	char	name[256];
	...
	if ((remnum<10) || (remnum>MAX_USERS))
	{
		return;
	}
	*/

	for (nottop=11;nottop<MAX_USERS;nottop++) {
		remnum = nottop;
...
void ACMD(edict_t *ent)
{
	...
	else if (strcmp(gi.argv(1),"reset_nottop10")==0)
	{
		if (ent->client->resp.admin<aset_vars->ACMD_RESET_LEVEL)
		return;
		delete_nottop10(ent);
	...
	else if (strcmp(gi.argv(1),"RemPlayer")==0)
	{
		RemPlayer(ent);
	}
...
/*	if (gi.argc() < 2) {
	...
	if (require_max && i>0 && map_added_time+i>gset_vars->voteextratime && curclients>1)
	{
		gi.cprintf(ent, PRINT_HIGH, "You may not vote more than %d minutes extra time per map.\n",gset_vars->voteextratime);
		return;
	}
	if (require_max && curclients<2 && map_added_time+i>gset_vars->voteextratime && map_added_time - level.time < 120.0) 
	{
		i = 5;	
	}
...
// draxi - timeleft, used it to fix the votetime->timeleft bug!
void timeleft(void)
{
	gi.bprintf(PRINT_HIGH,"%d\n",((mset_vars->timelimit + map_added_time) - ((int)(level.time / 60))));
}
//
...
void Changename(edict_t *ent)
{
	...
	// draxi: Neur0 said he would like to be able to change someones name into a name that alreayd existed!
	if (newid>=0)
	{
		if (gi.argc() < 5 && gi.argv(4)!="yes") {
			gi.cprintf(ent,PRINT_HIGH,"Are you sure you want to change new_name to a name that already exists?\n Write <<acmd changename orig_name new_name yes>> to do so\nWarning: Any name change will force the current map to be reloaded.");
			return;
		}
	}
...
//draxi : Delete a player
void RemPlayer(edict_t *ent)
{
	FILE	*f;
	char origname[255];
	char name[256];
	...
	if (gi.argc() < 3) 
	{
		gi.cprintf(ent,PRINT_HIGH,"format: acmd RemPlayer playername \nWarning: Any name change will force the current map to be reloaded.");
		return;
	}
	...
	for (mid=0;mid<maplist.nummaps;mid++) 
	{
		for (i=0;i<MAX_USERS;i++)
			{
			if (maplist.times[mid][i].uid==origid)
				{

					//gi.cprintf(ent,PRINT_HIGH,"Player found!\n"); //draxi testing
					maplist.times[mid][i].time = 0;
					maplist.times[mid][i].uid = -1;
					maplist.times[mid][i].date[0] = 0;
					maplist.times[mid][i].completions = 0;
					maplist.times[mid][i].fresh = false;

					if (maplist.times[mid][i].uid<MAX_HIGHSCORES) {
						for (i2=i+1;i2<=MAX_HIGHSCORES+1;i2++) {
							maplist.times[mid][i].time = maplist.times[mid][i2].time;
							maplist.times[mid][i].uid = maplist.times[mid][i2].uid;
							maplist.times[mid][i].date[0] = maplist.times[mid][i2].date[0];
							maplist.times[mid][i].completions = maplist.times[mid][i2].completions;
						}
					}
				
					//remove demos
					#ifdef ANIM_REPLAY
						sprintf (name, "%s/jumpdemo/%s.dj2", tgame->string,maplist.mapnames[mid]);
					#else
						sprintf (name, "%s/jumpdemo/%s.dj1", tgame->string,maplist.mapnames[mid]);
					#endif
					remove(name);	
					sprintf (name, "%s/jumpdemo/%s_%i.dj3", tgame->string,maplist.mapnames[mid],origid);
					remove(name);	
		...
		//remove from user.t
		maplist.users[origid].name[0] = 0;
		maplist.users[origid].score = 0;
		maplist.users[origid].lastseen = 0;
		for (j=0;j<10;j++)
		{
			maplist.users[origid].points[j] = 0;
		}


		//remove player.u
		sprintf (name, "%s/%s/%i.u", tgame->string,port->string,origid);
		remove(name);
	...
	gi.bprintf(PRINT_CHAT,"%s was deleted by an admin.\nRestarting map to save changes.\n",origname);
	strncpy(level.forcemap, level.mapname, sizeof(level.forcemap) - 1);
	EndDMLevel();
}
